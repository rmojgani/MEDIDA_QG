def init_net(SENSOR_RATIO, N, N2, D_Dx, D_Dy, Lx, Ly):
    ...
    #%%
    X = np.linspace(a,b,Nx,endpoint=ENDPOINT).reshape((Nx,1))
    Y = np.linspace(a,b,Ny,endpoint=ENDPOINT).reshape((Ny,1))
    ...
    #Niy = 2016
    #%%
    XX = np.linspace(a,b,N,endpoint=ENDPOINT).reshape((N,1))
    YY = np.linspace(a,b,N2,endpoint=ENDPOINT).reshape((N2,1))

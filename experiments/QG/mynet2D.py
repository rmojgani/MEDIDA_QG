...
#                break 
            # print history
            if epoch % int(NUM_EPOCHS_ADAM/50) == 0:    # print 50 times
                #print('[%d, %5d] loss: %.3e,  %.3e,  %.3e,  %.3e' %
                #       (epoch + 1, step + 1, loss, loss1, loss2lay1, loss2lay2))
                  losscpu = loss.detach().cpu().numpy()
                  print('[%d, %5d] loss: %.5e' %(epoch + 1, step + 1, losscpu) )
...
#        return mean_absolute_error
    ...
    def fft_loss(self, output, target, LAMBDA_REG):
        ...
        #loss = (1.0-LAMBDA_REG)*loss1 + LAMBDA_REG*loss2
        #return loss, loss1, loss2lay1, loss2lay2

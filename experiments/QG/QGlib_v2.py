...
def case_select( CASE_NO, CASE_NO_R, xx, yy):
    ...
    CONSTANTS_r = {
        ...
        'tau_d1':0.01,         #Newtonian relaxation time-scale for interface
        'tau_d2':0.01,         #Newtonian relaxation time-scale for interface
    ...
    if CASE_NO == 1001:
        CONSTANTS_m = {
            'nu':pow( 10., -6. ), #viscous dissipation
            'tau_d1':0.11,         #Newtonian relaxation time-scale for interface
            'tau_d2':0.11,         #Newtonian relaxation time-scale for interface
            'tau_f':0.07,         #surface friction 
            'beta' :0.196,        #beta 
            'sigma':3.5,
            'c_me_jac':1.0
        ...
        CONSTANTS_m = {
                'nu':pow( 10., -6. ), #viscous dissipation
                'tau_d1':0.0,         #Newtonian relaxation time-scale for interface
                'tau_d2':0.0,         #Newtonian relaxation time-scale for interface
                'tau_f':0.07,         #surface friction 
                'beta' :0.196,        #beta 
                'sigma':3.5,
                'c_me_jac':1.0
        ...
        CONSTANTS_m = {
            'nu':pow( 10., -6. ),   #viscous dissipation
            'tau_d':0.01,           #Newtonian relaxation time-scale for interface
            'tau_d1':0.01,           #Newtonian relaxation time-scale for interface (ψ1-ψ2)
            'tau_d2':0.01,          #Newtonian relaxation time-scale for interface  ψR
            'tau_f':0.07,           #surface friction, ∇2ψ
            'beta' :0.0,            #beta βy: Coriolis force
            'sigma':3.5,            # std in Jet in \psiR
            'c_me_jac':1.0  
            }
    print('Reference:', CONSTANTS_r)
    print('Model    :', CONSTANTS_m)
...
#    # beta * v
    ...
    forc1 = (psi_1[1] - psi_2[1])* tau_d1 - (psi_R[:, np.newaxis]) * tau_d2
    forc2 = -(psi_1[1] - psi_2[1])* tau_d1 + (psi_R[:, np.newaxis]) * tau_d2
...
#######################################################
#  Library| copied from interp2_newcases
def psi_lib(N2, N, Grad, Lapl, Grad8,
            D_Dx  , D_Dy,
            psic_1, psic_2, psi_R,
            vorc_1, vorc_2, 
...
#    icut = 0
    idx = 9
    X_labels = []
    # XX_2D = np.zeros((7+1+10, N2, N))
    # XX_2D = np.zeros((7+1+10+2+2, N2, N))
    # XX_2D = np.zeros((7+2+1, N2, N))# working 
#    XX_2D = np.zeros((2, N2, N))
#    XX_2D = np.zeros((7, N2, N))
    XX_2D = np.zeros((10, N2, N))
    ...
    if XX_2D.shape[0] > 7: 
        # J1 :
        X_labels.append(r'7: $J1$')
        XX_2D[7] = np.fft.irfft2(calc_jac1_psi(D_Dx, D_Dy, Lapl, psic_1, psic_2, 1, 0, 1))
        # J2 :
        X_labels.append(r'8: $J2$')
        XX_2D[8] = np.fft.irfft2(calc_jac2_psi(D_Dx, D_Dy, Lapl, psic_1, psic_2, 2, 0, 1))    
        # y :
        X_labels.append(r'9: $y$')
        XX_2D[9] =  y[:, np.newaxis]-0*XX_2D[8]
    ...
    # # grad psi 1:
    # idx= idx + 1
    # X_labels.append(str(idx)+r': $∇^4 ψ_1$')
    # XX_2D[idx] = np.fft.irfft2( Lapl * Lapl * psic_1 )
    # # grad psi 2:
    # idx= idx + 1
    # X_labels.append(str(idx)+r': $∇^4 ψ_2$')
    # XX_2D[idx] = np.fft.irfft2( Lapl * Lapl * psic_2 )
    
    # # grad psi 1:
    # idx= idx + 1
    # X_labels.append(str(idx)+r': $∇^8ψ_1$')
    # XX_2D[idx] = np.fft.irfft2(  Grad8 * psic_1 )
    # # grad psi 2:
    # idx= idx + 1
    # X_labels.append(str(idx)+r': $∇^8 ψ_2$')
    # XX_2D[idx] = np.fft.irfft2(  Grad8 * psic_2 )
